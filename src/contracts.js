...
class ContractGenerator {
  constructor(outputDir = path.join(__dirname, 'contracts')) {
  ...
  async generateSubcontractorAgreement(booking, gcInfo, subInfo) {
    ...
    return new Promise((resolve, reject) => {
      ...
      doc.text(`Client:`, { continued: false });
      doc.fontSize(9);
      ...
      const terms = [
        'The Service Provider agrees to perform the work in a professional and workmanlike manner.',
        'The Service Provider shall provide all necessary tools, equipment, and materials unless otherwise specified.',
        'The Service Provider shall maintain all required licenses, permits, and insurance coverage.',
        'The Service Provider agrees to comply with all applicable laws, codes, and regulations.',
        ...
        'This agreement was negotiated and confirmed via the BlueCollarClaw protocol.',
      ];
      ...
      doc.fontSize(9);
      doc.text('By accepting this agreement via BlueCollarClaw, both parties acknowledge and agree to the terms above.');
      doc.moveDown(1);

      doc.text(`Client Digital Signature: ACCEPTED via BlueCollarClaw on ${new Date().toLocaleDateString()}`);
      doc.moveDown(0.5);
      doc.text(`Service Provider Digital Signature: ACCEPTED via BlueCollarClaw on ${new Date().toLocaleDateString()}`);
      doc.moveDown(2);

      // Footer
      doc.fontSize(8).text('Generated by BlueCollarClaw - Autonomous Contractor Negotiation Protocol', {
        align: 'center',
      });
      doc.text(`BlueCollarClaw Booking ID: ${booking.id}`, { align: 'center' });
