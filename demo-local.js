...
console.log('=== ClawShake Local Demo (No MQTT) ===\n');
console.log('Simulating a GC finding a plumber via ClawShake...\n');
...
async function demo() {
  ...
  console.log('📋 Step 1: Creating Contractor Profiles\n');
  ...
  console.log(`  ✅ Created GC: Austin General Construction (${gcId})`);
  ...
  console.log(`  ✅ Created Plumber: Mike's Plumbing Co (${plumberId})`);
  console.log();

  await sleep(500);

  // Create job request
  console.log('📢 Step 2: GC Broadcasts Job Request\n');
  ...
  // Plumber evaluates
  console.log('🤖 Step 3: Plumber Agent Evaluates Request\n');
  ...
  console.log(`     Match: ${evaluation.matches ? '✅ YES' : '❌ NO'}`);
  ...
  console.log('🧠 Step 4: AI Decision\n');
  ...
  // Create offer
  console.log('💼 Step 5: Plumber Sends Offer\n');
  ...
  // GC accepts
  console.log('✅ Step 6: GC Accepts Offer\n');

  // Get GC contractor for reputation
  await new Promise((resolve) => {
    db.getContractorReputation(plumberId, (err, reputation) => {
      console.log('  📊 Plumber Reputation:');
      if (reputation && reputation.total_ratings > 0) {
        console.log(`     Rating: ${reputation.average_score}★ (${reputation.total_ratings} jobs)`);
      } else {
        console.log('     Rating: No ratings yet (new to ClawShake)');
      }
      resolve();
    });
  });
  ...
  console.log();
  console.log('  💬 GC Decision: ACCEPT');
  console.log();

  await sleep(1000);

  // Create booking
  console.log('📝 Step 7: Generate Contract & Calendar Event\n');
  ...
  console.log(`  📄 Contract Generated: ${contractPath}`);
  ...
  console.log('🎉 Step 8: Booking Confirmed!\n');
  console.log('  ═══════════════════════════════════════════════');
  ...
  console.log(`  Rate: $${offer.rate}/hr`);
  console.log(`  Estimated Hours: 24 (3 days × 8 hrs)`);
  ...
  console.log(`  ⚡ Time to Match: ${stats.timeToMatch}`);
  console.log(`  🔄 Negotiation Rounds: ${stats.negotiationRounds}`);
  console.log(`  👤 Human Interactions: ${stats.humanInteractions}`);
  console.log(`  ⏱️  Time Saved: ${stats.timeSaved}`);
  console.log();
