...
class ClawShakeAgent {
  constructor(contractorId, config = {}) {
    this.contractorId = contractorId;
    this.config = config;
    
    // Initialize components
    this.db = new Database(config.dbPath || './clawshake.db');
    this.network = null;
    this.negotiation = new NegotiationEngine(this.db, contractorId);
    this.contracts = new ContractGenerator(config.contractsDir || './contracts');
  ...
  async initialize() {
    ...
    // Connect to network
    this.network = new ClawShakeNetwork(
    ...
    console.log(`ClawShake agent ${this.contractorId} initialized and listening`);
  }
    ...
    this.db.getJobRequest(offer.requestId, async (err, request) => {
      ...
      this.db.getContractorReputation(offer.contractorId, (err, reputation) => {
        ...
        console.log(`[OFFER RECEIVED] From ${offer.contractorId}:`, {
          ...
          reputation: reputation ? `${reputation.average_score}★ (${reputation.total_ratings} jobs)` : 'No ratings yet',
        });
  ...
  async shutdown() {
    ...
    console.log('ClawShake agent shut down');
  }
}

module.exports = ClawShakeAgent;
